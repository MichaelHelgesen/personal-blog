    ...
    <div className={blogStyles.wrapperInner}>
        ...
        {pageContext.allCategories.map(cat => (
              ...
              <div>
                ...
                <p>
                  ...
                  {node.category.map(cat => (
                    cat ? <Link to={`/blog/category/${cat["categoryName"].toLowerCase()}`}>{cat["categoryName"].toLowerCase()}</Link> : null
                  ))}
                </p>
              </div>
            )
          ...
          {Array.from({ length: pageContext.numPages }).map((item, i) => {
            const index = i + 1
            const category = pageContext.category["categoryName"]
            const link =
              index === 1
                ? `/bloger/category/${category}`
                : `/bloger/category/${category}/page/${index}`
            return (
              <li>
